...
const SignUp = ({}) => {
  ...
  const signUpPassword = async (password, me, day1, time1, day2, time2) => {
    try {
      const {data:{data},} = await instance.post("/staff/signup", {"name": me, "password": password, "time": [{"day":day1, "time": time1}, {"day": day2, "time": time2}]})
      return data
    } catch (error) {
      ...
      <Input.Search
        ...
        onSearch={async (me) => {
          if (!me || !password || !day1 ||!time1 || !day2 ||!time2) {
            ...
            const message =  await signUpPassword(password, me, day1, time1, day2, time2)
              if(message !== "success")
              {
                displayStatus({
                  type: "error",
